...
public class PurchaseBills implements GenericController {
            ...
            if (tableView.getSelectionModel().getSelectedItems().size() == 1) {
                ...
                if (toDeletePurchaseBill != null &&
                        AlertMaker.showMCAlert("Confirm ? "
                                ...
                                , mainApp)) {
                    if (PurchaseBillService.deletePurchaseBill(toDeletePurchaseBill)) {
                ...
                if (AlertMaker.showMCAlert("Confirm ? "
                    ...
                    for (PurchaseBill toDeleteBill : toDelete)
                        PurchaseBillService.deletePurchaseBill(toDeleteBill);
                    loadTable();
                }
            }
        }
    ...
    public void handleEditPurchaseBill() {
        if (mainApp.getUser().getAccess().equals(StringUtil.ADMIN)) {
            if (editPanelCheckBox.isSelected()) {
                ...
                VBox vBox1 = singlePurchaseBillController.getVBox();
        ...
        if (mainApp.getUser().getAccess().equals("admin"))
            if (singlePurchaseBillController.isReady())
                if (AlertMaker.showMCAlert("Confirm?", "Are you sure you want to update these changes?", mainApp))
                    if (PurchaseBillService.updatePurchaseBill(singlePurchaseBillController.getPurchaseBill())) {
    ...
    private void loadTable() {
        ...
        if ((searchBox.getText() != null && !searchBox.getText().isEmpty()) || (searchBox2.getText() != null && !searchBox2.getText().isEmpty())) {
            if (searchBox.getText() != null && !searchBox.getText().isEmpty())
                purchaseBills = PurchaseBillService.getPurchaseBillList(searchBox.getText());
            else purchaseBills = PurchaseBillService.getPurchaseBillListByTotalNetAmount(searchBox2.getText());

        } else if (companyNameCBOX.getValue() != null)
            if (fromDate.getValue() != null && toDate.getValue() != null)
                purchaseBills = PurchaseBillService.getPurchaseBillList(companyNameCBOX.getValue(), fromDate.getValue(), toDate.getValue());
            else
                purchaseBills = PurchaseBillService.getPurchaseBillListByCompanyName(companyNameCBOX.getValue());
        else if (fromDate.getValue() != null && toDate.getValue() != null)
            purchaseBills = PurchaseBillService.getPurchaseBillList(fromDate.getValue(), toDate.getValue());
        else
            purchaseBills = PurchaseBillService.getAllPurchaseBillList();
    ...
    public void handleSendToAuditorSubmit() {
        ...
        for (PurchaseBill purchaseBill : selectedBills)
            result = result && PurchaseBillService.markBillAsGoneToAuditor(purchaseBill);
        loadTable();
    }
