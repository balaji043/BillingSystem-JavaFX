...
public class Product extends RecursiveTreeObject<Product> {
    ...
    public Product(String name, String hsn, String qty, String tax, String rate, String per) {
        this.name = name;
        this.hsn = hsn;
        this.qty = qty;
        this.tax = tax;
        ...
        float gstAmountFloat = originalAmount - originalAmount * (100 / (100 + t));

        singleOrg = String.format("%.2f", r - (r - r * (100 / (100 + t))));

        halfTaxPer = String.format("%.2f", t / 2);
        halfTaxAmt = String.format("%.2f", gstAmountFloat / 2);
        totalAmount = String.format("%.2f", originalAmount);
        this.gstAmount = String.format("%.2f", gstAmountFloat);
        ready = true;
