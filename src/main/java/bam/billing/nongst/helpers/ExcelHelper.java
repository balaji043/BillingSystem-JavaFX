...
import static bam.billing.nongst.utils.Constants.PURCHASE_BILLS_FOR;
...
public class ExcelHelper {
    ...
    public static boolean writeExcelBills(File dest, ObservableList<Bill> bills) {
        okay = false;
        try {
            ...
            row.createCell(3).setCellValue("Rounded Off");
            ...
            for (Bill bill : bills) {
                ...
                row.createCell(3).setCellValue(bill.getRoundOff());
                row.createCell(4).setCellValue("" + Math.ceil((Float.parseFloat(bill.getTotalAmount()))));
    ...
    public static boolean writeExcelCustomer(File dest, ObservableList<Customer> customers) {
        okay = false;
        try {
            String fileName = dest.getAbsolutePath();
            FileInputStream excel;
    ...
    public static boolean writeExcelPurchaseBills(File dest) {
        okay = false;
        try {
            String fileName = dest.getAbsolutePath();
            FileInputStream excel;
    ...
    public static boolean writeExcelPurchaseBills(File dest, String companyName, ObservableList<PurchaseBill> purchaseBills) {
        okay = false;
        try {
            String fileName = dest.getAbsolutePath();
            FileInputStream excel;
