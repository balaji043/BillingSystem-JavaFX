...
import bam.billing.nongst.controllers.NonGstBillController;
import bam.billing.nongst.controllers.SettingsController;
...
import static bam.billing.nongst.utils.ResourceConstants.Views;
...
public class AlertMaker {
    ...
    //Alert Dialog Box
    public static boolean showMCAlert(Message message, Main main) {
        return showMCAlert(message.title, message.title, main);
    }
    ...
    private static VBox getPane(Bill bill, Main mainApp) {
        ...
        try {
            VBox vBox;
            FXMLLoader loader = BillingSystemUtils.getFXMLLoader(Views.NON_GST_BILL);
            vBox = loader.load();
            NonGstBillController billingController = loader.getController();
    ...
    public static boolean showSettings(Main main) {
        try {
            ...
            dialogLayout.setStyle(String.format("-fx-background-color:%s;", Preferences.getPreferences().getCssThemeName().equals("black")
                    ? "rgb(50,50,50)" : "white"));
            FXMLLoader loader = BillingSystemUtils.getFXMLLoader(Views.SETTINGS_PAGE);
